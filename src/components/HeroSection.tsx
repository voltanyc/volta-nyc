...
export default function HeroSection() {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-v-bg pt-16">
      ...
      <motion.div className="relative z-10 flex flex-col items-center text-center" style={{ scale: titleScale, y: titleY }}>
        <motion.div
          className="mb-6"
        ...
        >
          <Image src="/logo.png" alt="Volta" width={180} height={180} className="object-contain" priority />
        </motion.div>
        ...
        >
          VOLTA<br /><span className="text-v-green">NYC</span>
        </motion.h1>
      </motion.div>
      ...
      >
        <p className="font-body text-lg md:text-xl text-v-muted max-w-2xl leading-relaxed mb-3">
          High school and college students building websites, running social media, and writing grants for NYC small businesses.{" "}
          <span className="text-v-ink font-semibold">Free of charge.</span>
        </p>
        <p className="font-body text-sm text-v-muted/70 mb-8">A registered 501(c)(3) nonprofit organization.</p>
        <div className="flex gap-4 flex-wrap justify-center">
          <Link href="/join" className="bg-v-green text-v-ink font-display font-bold text-base px-8 py-4 rounded-full hover:bg-v-green-dark transition-all hover:scale-105 shadow-lg shadow-v-green/20">
            Apply to Join →
          </Link>
          <Link href="/partners" className="border-2 border-v-blue text-v-blue font-display font-bold text-base px-8 py-4 rounded-full hover:bg-v-blue hover:text-white transition-all">
            Partner With Us
          </Link>
        </div>
      </motion.div>
